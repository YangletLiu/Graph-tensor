...
void gIFT_d(float* d_U, int k, float* d_f, int m, float* d_F){
    matrixMultiply_d(d_f, 0, m, k, d_U, k, d_F);
}
...
void gIFT_3D_batched_d(float *d_U, int k, float* d_f, int m, int n, float *d_F){
	...
	matrixMultiplytensor_d(d_U, flag_U_T, k, k,d_f_trans, m,n, d_F_trans);
...
void gIFT_3D_based_d(float *d_U, int k, float* d_f, int m, int n, float *d_F){
	...
	transSliceToTubal_d(d_f,m, n, k, d_f_trans);
	for(int i=0; i<n; i++){
		matrixMultiply_d(d_U, flag_U_T, k, k, d_f_trans+i*k*m, m, d_F_trans+i*k*m);
	}
	transTubalToSlice_d(d_F_trans, d_F, m, n, k);
