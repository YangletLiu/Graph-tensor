...
//	start = clock();
	...
	if(0 == strcmp("batched", argv[4])){
		...
		case 2:
			...
			break;
		case 3:
			gShift_3D_batched_d(d_L, k, d_f, m, n, d_F);
			break;
		default:
			break;
		}
		...
		switch(d){
		case 1:
			gShift_d(d_L, k, d_f, m, d_F);
			break;
		case 2:
			gShift_d(d_L, k, d_f, m, d_F);
			break;
		case 3:
			gShift_3D_based_d(d_L, k, d_f, m, n, d_F);
			break;
		default:
			break;
		}
	...
	switch(d){
	case 1:
		printf("gShift_%dD_%s of G[%d*%d] time: %lfs\n", d, argv[4], k, m, time);
		break;
	case 2:
		printf("gShift_%dD_%s of G[%d*%d] time: %lfs\n", d, argv[4], k, m, time);
		break;
	case 3:
		printf("gShift_%dD_%s of G[%d*%d*%d] time: %lfs\n", d, argv[4], m, n, k, time);
		break;
	default:
		break;
	}
...
/*    cudaFree(d_U);
	...
	//float *d_S = NULL;
	//float *d_V = NULL;
	Check(cudaMalloc((void**)&d_W, sizeof(float)*k*k));
	//Check(cudaMalloc((void**)&d_U, sizeof(float)*k*k));
	//Check(cudaMalloc((void**)&d_S, sizeof(float)*k));
	//Check(cudaMalloc((void**)&d_V, sizeof(float)*k*k));
	Check(cudaMemcpy(d_W, W, sizeof(float)*k*k, cudaMemcpyHostToDevice));
	//SVD(d_L, k, k, d_U, d_S, d_V);
	//Eig(d_L, k, d_U, d_S);
	//cudaDeviceSynchronize();
#if 0
    ...
    printf("\n+++++++++++++++++++++++++++++++++++++++++++\n");
#endif
    gShift_d(d_W, k, d_f, m, d_F);
	finish = clock();
	double time = (double)(finish-start) / CLOCKS_PER_SEC;
	printf("gShift_1D of G[%d*%d] time: %lfs\n", k, m, time);
