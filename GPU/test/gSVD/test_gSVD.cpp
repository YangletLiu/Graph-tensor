...
//	start = clock();
	...
	const int d = atoi(argv[2]);//图的每个节点的数据维度
	...
	float* A = (float*)malloc(sizeof(float)*m*n*k);//A graph-tensor
	for(int i=0; i< m*n*k; i++)
		A[i] = random(1000);

	//start = clock();
	float *d_A = NULL;
	Check(cudaMalloc((void**)&d_A, sizeof(float)*m*n*k));
	Check(cudaMemcpy(d_A, A, sizeof(float)*m*n*k, cudaMemcpyHostToDevice));
	...
	float* d_AS = NULL;
	float* d_AV = NULL;
	Check(cudaMalloc((void**)&d_AU, sizeof(float)*k*m*((m<n)?m:n)));
	Check(cudaMalloc((void**)&d_AS, sizeof(float)*((m<n)?m:n)*k));
	Check(cudaMalloc((void**)&d_AV, sizeof(float)*k*n*((m<n)?m:n)));
        
//	start = clock();
	if(0 == strcmp("batched", argv[3])){
		switch(d){
		case 1:
		...
		case 2:
			...
			break;
		case 3:
			gSVD_3D_batched_d(d_U, k, d_A, m, n, d_AU, d_AS, d_AV);
			break;
		default:
			break;
		}
	}
	else if(0 == strcmp("based",argv[3])){
		switch(d){
		case 1:
		...
		case 2:
			...
			break;
		case 3:
			gSVD_3D_based_d(d_U, k, d_A, m, n, d_AU, d_AS, d_AV);
			break;
		default:
			break;
		}
	...
	switch(d){
	case 1:
		printf("gSVD_%dD_%s of G[%d*%d] time: %lfs\n", d, argv[3], k, m, time);
		break;
	case 2:
		printf("gSVD_%dD_%s of G[%d*%d] time: %lfs\n", d, argv[3], k, m, time);
		break;
	case 3:
		printf("gSVD_%dD_%s of G[%d*%d*%d] time: %lfs\n", d, argv[3], m, n, k, time);
		break;
	default:
		break;
	}
