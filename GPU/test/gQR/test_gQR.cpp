...
//	start = clock();
	...
	float* A = (float*)malloc(sizeof(float)*m*n*k);//f 为图信号
	...
	float* d_AR = NULL;
	//float* d_AV = NULL;
	Check(cudaMalloc((void**)&d_AQ, sizeof(float)*k*m*n));
	Check(cudaMalloc((void**)&d_AR, sizeof(float)*k*n*n));
	...
	if(0 == strcmp("batched", argv[4])){
		...
		case 2:
			...
			break;
		case 3:
			gQR_3D_batched_d(d_U, k, d_A, m, n, d_AR);
			break;
		default:
			break;
		}
		...
		case 2:
			...
			break;
		case 3:
			gQR_3D_based_d(d_U, k, d_A, m, n, d_AQ, d_AR);
			break;
		default:
			break;
		}
	...
	switch(d){
	case 1:
		printf("gQR_%dD_%s of G[%d*%d] time: %lfs\n", d, argv[4], k, m, time);
		break;
	case 2:
		printf("gQR_%dD_%s of G[%d*%d] time: %lfs\n", d, argv[4], k, m, time);
		break;
	case 3:
		printf("gQR_%dD_%s of G[%d*%d*%d] time: %lfs\n", d, argv[4], m, n, k, time);
		break;
	default:
		break;
	}
