...
//	start = clock();
	...
	if(0 == strcmp("batched", argv[4])){
		switch(d){
		case 1:
			//gIFT_3D_batched_d(d_U, k, d_f, m, n, d_F);
			break;
		case 2:
			//gIFT_3D_batched_d(d_U, k, d_f, m, n, d_F);
			break;
		...
		switch(d){
		case 1:
			gIFT_d(d_U, k, d_f, m, d_F);
			break;
		case 2:
			gIFT_d(d_U, k, d_f, m, d_F);
			break;
		case 3:
			gIFT_3D_based_d(d_U, k, d_f, m, n, d_F);
			break;
		default:
			break;
		}
	...
	switch(d){
	case 1:
		printf("gIFT_%dD_%s of G[%d*%d] time: %lfs\n", d, argv[4], k, m, time);
		break;
	case 2:
		printf("gIFT_%dD_%s of G[%d*%d] time: %lfs\n", d, argv[4], k, m, time);
		break;
	case 3:
		printf("gIFT_%dD_%s of G[%d*%d*%d] time: %lfs\n", d, argv[4], m, n, k, time);
		break;
	default:
		break;
	}
	...
	finish = clock();
	double time = (double)(finish-start) / CLOCKS_PER_SEC;
	printf("gIFT_1D of G[%d*%d] time: %lfs\n", k, m, time);
